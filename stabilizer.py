...
class stabilizer:
    ...
    def add_branch(self, node, thechar):
        if thechar == 'I':
            ...
            print('Undefined Symbol!',thechar)
            sys.exit()       
    ...
    def get_str_from_tree(self, curnode, s, l):
        if len(s) < l:
            ...
            return
        return
    def build_ops_from_tree(self):
        self.tmp_opr = []
        self.tmp_data = []
        curnode = self.tree
        self.get_str_from_tree(curnode, '', len(self.operator[0]))
        return self.tmp_opr, self.tmp_data
        ...
        for i in foldarray:
            if self.is_term_exist(i):
                ...
                if qbit[1] == 'I':
                    ...
                    print('No such symbol!', qbit[1])
                    sys.exit()
        return
    def run(self):
        self.build_tree_from_op()
        self.operator, self.data = self.build_ops_from_tree()
        ...
        while len(self.operator) > 2:
            ...
            if len(qbits) > 1:
                ...
                print('Choose',qbits[0][1],'to be',mp)
